...
describe('NegocioService', () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<Repository<NegocioEntity>>(
  ...
  it('create deberia retornar un negocio', async () => {
    ...
    const storedNegocio: NegocioEntity = await repository.findOne({
  ...
  it('update deberia modificar un negocio', async () => {
    ...
    const updatedNegocio: NegocioEntity = await service.update(
      negocio.id,
      negocio,
    );
  ...
  it('update deberia mandar exception si el negocio no es valido', async () => {
    ...
    await expect(() => service.update('0', negocio)).rejects.toHaveProperty(
      'message',
      'El negocio con el id dado no fue encontrado',
    );
  });
  ...
  it('delete deberia remover un negocio', async () => {
    ...
    const deletedNegocio: NegocioEntity = await repository.findOne({
      where: { id: negocio.id },
    });
