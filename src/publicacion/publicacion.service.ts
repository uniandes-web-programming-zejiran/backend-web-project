...
export class PublicacionService {
    ...
    async findOne(id: string): Promise<PublicacionEntity> {
        const publicacion: PublicacionEntity = await this.publicacionRepository.findOne({where: {id}, relations: ["usuario"]});
        if (!publicacion) {
            throw new BusinessLogicException("La publicacion con el id dado no fue encontrado", BusinessError.NOT_FOUND);
        }
        return publicacion;
    }
    ...
    async update(id: string, publicacion: PublicacionEntity): Promise<PublicacionEntity> {
        const persistedPublicacion: PublicacionEntity = await this.publicacionRepository.findOne({where: {id}});
        if (!persistedPublicacion) {
            throw new BusinessLogicException("La publicacion con el id dado no fue encontrado", BusinessError.NOT_FOUND);
        }
        return await this.publicacionRepository.save({...persistedPublicacion, ...publicacion});
    }

    //Eliminar una publicacion por su id
    async delete(id: string) {
        const publicacion: PublicacionEntity = await this.publicacionRepository.findOne({where: {id}});
        if (!publicacion) {
            throw new BusinessLogicException("La publicacion con el id dado no fue encontrado", BusinessError.NOT_FOUND);
        }
