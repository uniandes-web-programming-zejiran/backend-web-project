...
describe('PublicacionService', () => {
    ...
    beforeEach(async () => {
        ...
        repository = module.get<Repository<PublicacionEntity>>(getRepositoryToken(PublicacionEntity));
        await seedDatabase();
    });
    ...
    const seedDatabase = async () => {
        ...
        for (let i = 0; i < 2; i++) {
            const publicacion: PublicacionEntity = await repository.save({
                ...
                fechaPubliacion: faker.date.past().toString(),
    ...
    it('findOne deberia retornar una publicacion por id', async () => {
        ...
        const publicacion: PublicacionEntity = await service.findOne(storedPublicacion.id);
        ...
        expect(publicacion.fechaPubliacion).toEqual(storedPublicacion.fechaPubliacion)
    ...
    it('create deberia crear una publicacion', async () => {
        ...
        const storedPublicacion: PublicacionEntity = await repository.findOne({where: {id: newPublicacion.id}});
        ...
        expect(storedPublicacion.fechaPubliacion).toEqual(newPublicacion.fechaPubliacion)
    ...
    it('update deberia actualizar una publicacion', async () => {
        ...
        const updatedPublicacion: PublicacionEntity = await service.update(publicacion.id, publicacion);
        expect(updatedPublicacion).not.toBeNull();
        const storedPublicacion: PublicacionEntity = await repository.findOne({where: {id: publicacion.id}});
        expect(storedPublicacion).not.toBeNull();
    ...
    it('delete deberia eliminar una publicacion', async () => {
        ...
        const deletedPublicacion: PublicacionEntity = await repository.findOne({where: {id: publicacion.id}});
