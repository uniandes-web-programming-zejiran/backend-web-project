...
export class NegocioProductoService {
    ...
    async addProductoNegocio(negocioId: string, productoId: string): Promise<NegocioEntity> {
    ...
    async findProductoByNegocioIdProductoId(negocioId: string, productoId: string): Promise<ProductoEntity> {
        const producto: ProductoEntity = await this.productoRepository.findOne({where: {id: productoId}});
        if (!producto)
          throw new BusinessLogicException("El producto con el id dado no fue encontrado", BusinessError.NOT_FOUND)
        ...
        const negocioProducto: ProductoEntity = negocio.productos.find(e => e.id === producto.id);
   
        if (!negocioProducto)
          throw new BusinessLogicException("El producto con el id dado no esta asociado al negocio", BusinessError.PRECONDITION_FAILED)
   
        return negocioProducto;
    }
    
    async findProductosByNegocioId(negocioId: string): Promise<ProductoEntity[]> {
        const negocio: NegocioEntity = await this.negocioRepository.findOne({where: {id: negocioId}, relations: ["productos"]});
        if (!negocio)
          throw new BusinessLogicException("El negocio con el id dado no fue encontrado", BusinessError.NOT_FOUND)
    ...
    async associateProductosNegocio(negocioId: string, productos: ProductoEntity[]): Promise<NegocioEntity> {
        const negocio: NegocioEntity = await this.negocioRepository.findOne({where: {id: negocioId}, relations: ["productos"]});
