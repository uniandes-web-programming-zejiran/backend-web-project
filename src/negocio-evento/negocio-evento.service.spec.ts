...
describe('NegocioEventoService', () => {
  ...
  it('addEventoNegocio should add an evento to a negocio', async () => {
    ...
    const result: NegocioEntity = await service.addEventoNegocio(newNegocio.id, newEvento.id);
  ...
  it('addEventoNegocio should thrown exception for an invalid evento', async () => {
    ...
    await expect(() => service.addEventoNegocio(newNegocio.id, "0")).rejects.toHaveProperty("message", "El evento con el id dado no fue encontrado");
  });
  ...
  it('addEventoNegocio should throw an exception for an invalid negocio', async () => {
    ...
    await expect(() => service.addEventoNegocio("0", newEvento.id)).rejects.toHaveProperty("message", "El negocio con el id dado no fue encontrado");
  });

  it('findEventoByNegocioIdEventoId should return evento by negocio', async () => {
    const evento: EventoEntity = eventosList[0];
    const storedEvento: EventoEntity = await service.findEventoByNegocioIdEventoId(negocio.id, evento.id, )
  ...
  it('findEventoByNegocioIdEventoId should throw an exception for an invalid evento', async () => {
    await expect(()=> service.findEventoByNegocioIdEventoId(negocio.id, "0")).rejects.toHaveProperty("message", "El evento con el id dado no fue encontrado"); 
  });

  it('findEventoByNegocioIdEventoId should throw an exception for an invalid negocio', async () => {
    const evento: EventoEntity = eventosList[0]; 
    await expect(()=> service.findEventoByNegocioIdEventoId("0", evento.id)).rejects.toHaveProperty("message", "El negocio con el id dado no fue encontrado"); 
  });
  ...
  it('findEventoByNegocioIdEventoId should throw an exception for an evento not associated to the negocio', async () => {
    ...
    await expect(()=> service.findEventoByNegocioIdEventoId(negocio.id, newEvento.id)).rejects.toHaveProperty("message", "El evento con el id dado no esta asociado al negocio"); 
  });

  it('findEventosByNegocioId should return eventos by negocio', async ()=>{
    const eventos: EventoEntity[] = await service.findEventosByNegocioId(negocio.id);
    expect(eventos.length).toBe(5)
  });

  it('findEventosByNegocioId should throw an exception for an invalid negocio', async () => {
    await expect(()=> service.findEventosByNegocioId("0")).rejects.toHaveProperty("message", "El negocio con el id dado no fue encontrado"); 
  });
  ...
  it('associateEventosNegocio should update eventos list for a negocio', async () => {
    ...
    const updatedNegocio: NegocioEntity = await service.associateEventosNegocio(negocio.id, [newEvento]);
  ...
  it('associateEventosNegocio should throw an exception for an invalid negocio', async () => {
    ...
    await expect(()=> service.associateEventosNegocio("0", [newEvento])).rejects.toHaveProperty("message", "El negocio con el id dado no fue encontrado"); 
  });
  ...
  it('associateEventosNegocio should throw an exception for an invalid evento', async () => {
    ...
    await expect(()=> service.associateEventosNegocio(negocio.id, [newEvento])).rejects.toHaveProperty("message", "El evento con el id dado no fue encontrado"); 
  });
  ...
  it('deleteEventoToNegocio should remove an evento from a negocio', async () => {
    ...
    const storedNegocio: NegocioEntity = await negocioRepository.findOne({where: {id: negocio.id}, relations: ["eventos"]});
    const deletedEvento: EventoEntity = storedNegocio.eventos.find(a => a.id === evento.id);
  ...
  it('deleteEventoToNegocio should thrown an exception for an invalid evento', async () => {
    await expect(()=> service.deleteEventoNegocio(negocio.id, "0")).rejects.toHaveProperty("message", "El evento con el id dado no fue encontrado"); 
  });

  it('deleteEventoToNegocio should thrown an exception for an invalid negocio', async () => {
    const evento: EventoEntity = eventosList[0];
    await expect(()=> service.deleteEventoNegocio("0", evento.id)).rejects.toHaveProperty("message", "El negocio con el id dado no fue encontrado"); 
  });
  ...
  it('deleteEventoToNegocio should thrown an exception for an non asocciated evento', async () => {
    ...
    await expect(()=> service.deleteEventoNegocio(negocio.id, newEvento.id)).rejects.toHaveProperty("message", "El evento con el id dado no esta asociado al negocio"); 
  }); 
