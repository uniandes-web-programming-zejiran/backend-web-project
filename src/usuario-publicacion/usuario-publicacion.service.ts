...
export class UsuarioPublicacionService {
  ...
  async findPublicacionByUsuarioIdPublicacionId(
    ...
    const usuarioPublicacion: PublicacionEntity = usuario.publicaciones.find(
  ...
  async associatePublicacionUsuario(
    idUsuario: string,
    publicaciones: PublicacionEntity[],
  ): Promise<UsuarioEntity> {
    const usaurio: UsuarioEntity = await this.usuarioRepository.findOne({
      where: { id: idUsuario },
      relations: ['publicaciones'],
    });
    if (!usaurio) {
